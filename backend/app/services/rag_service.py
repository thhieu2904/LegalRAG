...
from .query_router import QueryRouter, RouterBasedAmbiguousQueryService
from .context_service import EnhancedContextExpansionService
...
class OptimizedEnhancedRAGService:
    ...
    def __init__(
        ...
        cached_router=None  # Optional cached router for faster startup
    ):
        self.documents_dir = documents_dir
        self.vectordb_service = vectordb_service
        self.llm_service = llm_service
        self.cached_router = cached_router
    ...
    def _initialize_services(self):
        """Khởi tạo các service hỗ trợ với Enhanced Smart Router hoặc Cached Router"""
        try:
            # Use cached router if available, otherwise create Enhanced Smart Router
            if self.cached_router:
                self.smart_router = self.cached_router
                logger.info("⚡ Using cached router for instant startup")
            else:
                # Enhanced Smart Query Router với Example Questions Database
                if self.vectordb_service.embedding_model:
                    self.smart_router = QueryRouter(embedding_model=self.vectordb_service.embedding_model, mode="enhanced")
                    logger.info("✅ Enhanced Smart Query Router initialized")
                else:
                    raise Exception("No embedding model available for router initialization")
    ...
    def enhanced_query(
        ...
        try:
            ...
            if use_ambiguous_detection:
                ambiguous_result = self.ambiguous_service.detect_ambiguous_query(query)
                
                if ambiguous_result.get('is_ambiguous', False):
                    self.metrics["ambiguous_detected"] += 1
                    logger.info(f"Ambiguous query detected: {ambiguous_result['category']} (confidence: {ambiguous_result['confidence']:.2f})")
                    
                    return {
                        "type": "clarification_needed",
                        "category": ambiguous_result['category'],
                        "confidence": ambiguous_result['confidence'],
                        "suggested_collections": ambiguous_result['suggested_collections'],
                        "message": "Câu hỏi của bạn có thể thuộc nhiều lĩnh vực. Bạn có thể làm rõ thêm không?",
                        "session_id": session_id,
                        "processing_time": time.time() - start_time
                    }
                else:
                    # Query không mơ hồ - có target collection rồi
                    route_result = ambiguous_result.get('route_result', {})
                    target_collection = route_result.get('collection_name')
                    best_collections = [target_collection] if target_collection else [settings.chroma_collection_name]
                    logger.info(f"Smart routed to collection: {target_collection} (confidence: {ambiguous_result['confidence']:.2f})")
            else:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "router_stats": self.smart_router.get_mode_info(),
                "context_expansion": {
